...
# ================================================================
# 6. 피쳐 선별 (동적 — importance 기반 최적 개수 탐색)
# ================================================================
print("\n--- 피쳐 선별 (importance 기반) ---")
...
# 1단계: 전체 피쳐로 모델 학습 → importance 추출
...
# 2단계: 피쳐 개수별 성능 비교 (20~55개)
best_r2 = -999
best_n = len(ALL_FEATURES)

for n_feat in range(20, min(56, len(ALL_FEATURES) + 1)):
    top_feats = importance.head(n_feat)['feature'].tolist()
    m = xgb.XGBRegressor(
...
print(f"  최적 피쳐 수: {best_n}개 (R²={best_r2:.4f})")
